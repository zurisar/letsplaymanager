...
# По умолчанию путь к стандартной БД (чтобы ничего не сломалось при запуске)
db_path = os.path.join(APP_DATA_DIR, "letsplay.db")

def set_db_filename(filename):
    """Принимает только имя файла (например letsplay_123.db) и строит полный путь внутри APP_DATA_DIR"""
    global db_path
    db_path = os.path.join(APP_DATA_DIR, filename)
...
def init_db():
    ...
    # Таблица для связи плейлистов и видеохостингов
    cursor.execute('''
...
def add_game(name, folder_path, ai_url="", steam_id="", release_date="", desc_template="", default_tags=""):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO games (name, folder_path, ai_url, steam_id, release_date, desc_template, default_tags) 
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ''', (name, folder_path, ai_url, steam_id, release_date, desc_template, default_tags))
    new_id = cursor.lastrowid # Получаем ID новой игры
    conn.commit()
    conn.close()
    return new_id
...
def get_games():
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute('SELECT id, name, folder_path, ai_url, steam_id, desc_template, default_tags FROM games')
    games = cursor.fetchall()
    conn.close()
    return games

def update_game(game_id, name, ai_url, steam_id, desc_template="", default_tags=""):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute('''
        UPDATE games 
        SET name = ?, ai_url = ?, steam_id = ?, desc_template = ?, default_tags = ? 
        WHERE id = ?
    ''', (name, ai_url, steam_id, desc_template, default_tags, game_id))
    conn.commit()
    conn.close()
...
def update_episode_metadata_full(episode_id, custom_title, custom_desc, timecodes, custom_tags):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute('''
        UPDATE episodes 
        SET custom_title = ?, custom_desc = ?, timecodes = ?, custom_tags = ? 
        WHERE id = ?
    ''', (custom_title, custom_desc, timecodes, custom_tags, episode_id))
    conn.commit()
    conn.close()
...
def get_episode_metadata(episode_id):
    ...
    cursor = conn.cursor()
    cursor.execute('''
        SELECT custom_title, custom_desc, timecodes, custom_tags 
...
def get_game_playlists(game_id):
    """Возвращает словарь { 'youtube': 'url', 'rutube': 'url' }"""
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute('''
        SELECT v.key, gp.playlist_url 
        FROM game_playlists gp
        JOIN videohostings v ON gp.videohosting_id = v.id
        WHERE gp.game_id = ?
    ''', (game_id,))
...
def update_game_playlists(game_id, playlists_dict):
    """Обновляет ссылки. playlists_dict = {'youtube': 'url', 'rutube': 'url'}"""
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    
    # Получаем id хостингов по их ключам
    cursor.execute('SELECT id, key FROM videohostings')
    hostings = {row[1]: row[0] for row in cursor.fetchall()}
    
    for key, url in playlists_dict.items():
        if key in hostings:
            h_id = hostings[key]
            # Используем UPSERT (вставка или обновление при конфликте ключей)
            cursor.execute('''
                INSERT INTO game_playlists (game_id, videohosting_id, playlist_url)
                VALUES (?, ?, ?)
                ON CONFLICT(game_id, videohosting_id) DO UPDATE SET playlist_url = ?
            ''', (game_id, h_id, url, url))
